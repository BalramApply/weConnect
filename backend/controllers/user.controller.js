  ...
  try {
    ...
    if (existingUser) {
      ...
      return res.status(409).json({
        success: false,
        field,
        message: `${field} "${req.body[field]}" is already taken`,
      });
    }
...
export const getUserAndProfile = async (req, res) => {
  try {
    ...
    return res.json({ profile: userProfile }); // ✅ wraps it in a "profile" key
...
export const updateProfileData = async (req, res) => {
  try {
    ...
    return res.json({ message: "Profilen Updated" });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
